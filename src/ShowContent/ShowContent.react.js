...
class ShowContent extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
           // response : {"total_count":0,"incomplete_results":false,"items":[{}]},
            //response : [],
            loaded : false,
            //url : "https://api.github.com/search/repositories?q=language:"+this.props.language+"&sort=stars&per_page=30"  // use template literal
    ...
    //     fetch(this.props.url).then(data => data.json()).
    //     then(response => {
    //         this.setState({response:response.items,loaded:true});
   ...
   // list
    render() {
        ...
        const items = []
           ...
           for (var index=0;index<(this.props.response.length-4);index +=4)
            {
                items.push(
                    <Table borderless hover variant="dark" style={{marginBottom : 0 }}>
                    <tbody >
                       <tr>
                         <td><CardTemplate key={index} list={this.props.response[index]}></CardTemplate></td>
                         <td><CardTemplate key={index} list={this.props.response[index+1]}></CardTemplate></td>
                         <td><CardTemplate key={index} list={this.props.response[index+2]}></CardTemplate></td>
                         <td><CardTemplate key={index} list={this.props.response[index+3]}></CardTemplate></td>
                      </tr>
                    </tbody>
                   </Table>
                    );
